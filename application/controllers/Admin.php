...
class Admin extends CI_Controller
{
    ...
    function manage_post()
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(site_url('login'), 'refresh');
        $page_data['page_name']  = 'manage_posts';
        $page_data['page_title'] = 'Manage Post';
        $this->load->view('backend/index', $page_data);
    }
    function edit_post($id = '')
    {
        ...
        $page_data['data']       = $this->db->get_where('post',array('post_id'=>$id))->row();
        $this->load->view('backend/index', $page_data);
    }

    function add_hall_of_fame()
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(site_url('login'), 'refresh');
        $page_data['page_name']  = 'add_hall_of_fame';
        $page_data['page_title'] = 'Add Hall of Fame';
        $this->load->view('backend/index', $page_data);
    }
    function manage_hall_of_fame()
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(site_url('login'), 'refresh');
        $page_data['page_name']  = 'manage_hall_of_fame';
        $page_data['page_title'] = 'Manage Hall of Fame';
        $this->load->view('backend/index', $page_data);
    }
    function edit_hall_of_fame($id = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(site_url('login'), 'refresh');
        $page_data['page_name']  = 'edit_hall_of_fame';
        $page_data['page_title'] = 'Edit Hall of Fame';
        $page_data['data']       = $this->db->get_where('hall_of_fame',array('id'=>$id))->row();
        $this->load->view('backend/index', $page_data);
    }

      function hall_of_fame($param1 = "", $param2 = "") {
        if ($this->session->userdata('admin_login') != 1)
            redirect(site_url('login'), 'refresh');
            ...
            if ($param1 == 'create') {
                ...
                $validation = $this->db->get_where('hall_of_fame',array('student_name' => $data['student_name'],'board' => $$data['board']))->num_rows();
                if ($validation > 0) 
                {

                    $this->session->set_flashdata('error_message' , 'Already exist');
                ...
                redirect(site_url('admin/add_hall_of_fame'), 'refresh');
            }
            ...
            if ($param1 == 'edit') {
                ...
                $data['percentage']      = $this->input->post('percentage');
                    ...
                    $this->db->update('hall_of_fame' , $data);
                    $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
                
                redirect(site_url('admin/manage_hall_of_fame'), 'refresh');
            }

            if ($param1 == 'delete') {
                $this->db->where('id' , $param2);
                $this->db->delete('hall_of_fame');
     ...
     function get_hall_of_fame() {
        ...
        $totalData = $this->ajaxload->all_hall_of_fame_count();
        $totalFiltered = $totalData;

        if(empty($this->input->post('search')['value'])) {
            $hall_of_fame = $this->ajaxload->all_hall_of_fame($limit,$start,$order,$dir);
        }
        else {
            $search = $this->input->post('search')['value'];
            $hall_of_fame =  $this->ajaxload->hall_of_fame_search($limit,$start,$search,$order,$dir);
            $totalFiltered = $this->ajaxload->hall_of_fame_search_count($search);
        }

        $data = array();
        if(!empty($hall_of_fame)) {
            foreach ($hall_of_fame as $row) {

              //  $photo = '<img src="'.$this->crud_model->get_image_url('teacher', $row->id).'" class="img-circle" width="30" />';

                $options = '<div class="btn-group"><button type="button" class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown">
                                    Action <span class="caret"></span></button><ul class="dropdown-menu dropdown-default pull-right" role="menu"><li><a href="'.base_url().'admin/edit_hall_of_fame/'.$row->id.'" ><i class="entypo-pencil"></i>&nbsp;'.get_phrase('edit').'</a></li><li class="divider"></li><li><a href="#" onclick="hall_of_fame_delete_confirm('.$row->id.')"><i class="entypo-trash"></i>&nbsp;'.get_phrase('delete').'</a></li></ul></div>';

                $nestedData['id'] = $row->id;
                $nestedData['student_name'] = $row->student_name;
                $nestedData['year'] = $row->year;
                $nestedData['board'] = $row->board;
                $nestedData['stream'] = $row->stream;
    ...
    function admission_approve()
    {
           ...
           $form_number = $this->input->post('form_number');
            ...
            $result = $this->db->update('admission',$data);

            if($result)
            {
                $returnArray['status'] = true;
                $returnArray['message'] = 'Approved';
 ...
 function post($param1 = "", $param2 = "") {
        if ($this->session->userdata('admin_login') != 1)
            ...
            if ($param1 == 'create') {
                ...
                $data['post_url']      = $this->input->post('url');
                $data['created']   = date('d-m-Y h:i:s');
                $data['updated']    = '';
                $data['category']    = '';
                 if ($_FILES['featured_image']['name'] != '') {
              $data['featured_image']  = $_FILES['featured_image']['name'];
              move_uploaded_file($_FILES['featured_image']['tmp_name'], 'uploads/post/'. $_FILES['featured_image']['name']);
            }

                    $this->db->insert('post', $data);
                    $this->session->set_flashdata('flash_message' , 'Post Added');
                   
                redirect(site_url('admin/add_post'), 'refresh');
            }
            ...
            if ($param1 == 'edit') {
              ...
              move_uploaded_file($_FILES['featured_image']['tmp_name'], 'uploads/post/'. $_FILES['featured_image']['name']);
                ...
                redirect(site_url('admin/edit_post/'.$param2), 'refresh');
            }
            ...
            if ($param1 == 'delete') {
                ...
                $this->session->set_flashdata('flash_message' , get_phrase('data_deleted'));
                redirect(site_url('admin/manage_post'), 'refresh');
            }
    ...
    function get_posts() {
        ...
        $totalData = $this->ajaxload->all_post_count();
        $totalFiltered = $totalData;

        if(empty($this->input->post('search')['value'])) {
            $post = $this->ajaxload->all_post($limit,$start,$order,$dir);
        }
        else {
            $search = $this->input->post('search')['value'];
            $post =  $this->ajaxload->post_search($limit,$start,$search,$order,$dir);
            $totalFiltered = $this->ajaxload->post_search_count($search);
        }
        ...
        if(!empty($post)) {
            foreach ($post as $row) {
                ...
                $photo = '<img src="'.base_url().'uploads/post/'.$row->featured_image.'" class="img-circle" height="90" />';

                $options = '<div class="btn-group"><button type="button" class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown">
                                    Action <span class="caret"></span></button><ul class="dropdown-menu dropdown-default pull-right" role="menu"><li><a href="'.base_url().'admin/edit_post/'.$row->post_id.'" ><i class="entypo-pencil"></i>&nbsp;'.get_phrase('edit').'</a></li><li class="divider"></li><li><a href="#" onclick="post_delete_confirm('.$row->post_id.')"><i class="entypo-trash"></i>&nbsp;'.get_phrase('delete').'</a></li></ul></div>';

                $nestedData['id'] = $row->post_id;
    ...
    //Admission start//
    function manage_science_admission($param1='',$param2='',$param3='')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(site_url('login'), 'refresh');

        $page_data['page_name']     = 'manage_science_admission';
        $page_data['page_title']    = get_phrase('Manage Science Admission');
        $this->load->view('backend/index', $page_data);
    }
  ...
  function student_profile($student_id)
  {
    ...
    function student_edit($student_id = '')
    {
        ...
        $page_data['page_title']    = 'Student Edit';
        $page_data['student_id']  = $student_id;
        $this->load->view('backend/index', $page_data);
    }
    ...
    function student($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            if($this->input->post('address_line1') != null){
              $data['address_line1']      = $this->input->post('address_line1');
            }
            if($this->input->post('address_line2') != null){
              $data['address_line2']      = $this->input->post('address_line2');
            }
            if($this->input->post('address_line3') != null){
            ...
            if($this->input->post('institute_code') != null){
    $data['institute_code']  =  $this->db->get_where('settings' , array('type'=>'institute_code'))->row()->description;
            }
             if($this->input->post('phone') != null){
              $data['phone']        = $this->input->post('phone');
            }
            
            $data['student_360_id']   = 'cs'.date('admyhis');
            
            if($this->input->post('admission_year') != null){
                $data['admission_year']   = $this->input->post('admission_year');
            }
        ...
        if ($param1 == 'do_update') {
            ...
            if($this->input->post('leave_year') != null){
...
*/
        ...
        if ($param1 == 'send_sms') {
            ...
            foreach ($students as $row) {
                ...
                foreach ($marks as $row2) {
                    ...
                    $message      .= 'Exam Result:  '.$row['name'].'' . $subject . ' : ' . $mark_obtained . ' ,';

                }
                // send sms
                $send_sms= $this->sms_model->send_sms( $message , $receiver_phone );
            }
    ...
    function noticeboard($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            if ($_FILES['file1']['name'] != '') {
              $data['file']  = $_FILES['file1']['name'];
              move_uploaded_file($_FILES['file1']['tmp_name'], 'uploads/frontend/noticeboard/'. $_FILES['file1']['name']);
            }
    ...
    function generate_bulk_student_csv($class_id = '', $section_id = '')
    {
        ...
        $line   = array('roll_number', 'first_name', 'middle_name', 'last_name', 'password', 'phone', 'address_line1', 'address_line2', 'address_line3');
        fputcsv($file, $line, ',');
       ...
       if ($param1 == 'import') {
          if ($this->input->post('class_id') != '' && $this->input->post('section_id') != '') {
             ...
             foreach ($csv as $row) {
                  ...
                  $data['password']  = sha1($row[4]);
                  $data['phone']     = $row[5];
                  $data['address_line1']   = $row[6];
                  $data['address_line2'] = $row[7];
                  $data['address_line3']  = $row[8];
                  $data['city']  = $row[9];
                  $data['region']  = $row[10];
                  $data['birthday']  = $row[11];
                  $data['admission_year']  = $row[12];
                  $data['email']  = $row[13];
                  $data['student_360_id'] = 'cs'.date('admyh').$student_code;
                  $data['institute_code'] = $this->crud_model->getRow('settings','type','institute_code','description');
