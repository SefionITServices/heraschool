    ...
    <div class="col-md-9">
      ...
      <li class="">
        ...
        <?php
          ...
          $basic_info_values = [$row['first_name'].' '.$row['middle_name'].' '.$row['last_name'], $row['parent_id'] == NULL ? '' : $this->db->get_where('parent', array('parent_id' => $row['parent_id']))->row()->name,
          $class_name, $section_name, $row['email'], $row['phone'] == NULL ? '' : $row['phone'], $row['address_line1'].' '.$row['address_line2'].' '.$row['address_line3'], $row['gender'] == NULL ? '' : $row['gender'], $row['birthday'],
